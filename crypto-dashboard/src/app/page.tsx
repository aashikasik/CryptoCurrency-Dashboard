...
function GlobalSearchBar({ onSearch }: { onSearch: (v: string) => void }) {
  ...
  useEffect(() => {
    ...
    // Fetch coins matching search
    fetchMarkets(1, 5, value, {})
      .then(setResults)
      .catch(() => setResults([]));
  }, [value]);
  return (
    <div className="fixed top-2 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-2 sm:px-0">
      <div className="bg-white dark:bg-gray-900 rounded shadow border flex flex-col gap-2 p-2 sm:p-4">
        <input
          ref={ref}
          type="text"
          value={value}
          ...
          className="w-full px-4 py-2 rounded border bg-white dark:bg-gray-900 dark:text-white text-sm sm:text-base"
        />
        {value && results.length > 0 && (
          <div className="flex flex-col gap-2">
            {results.map(coin => (
              <div key={coin.id} className="flex items-center justify-between gap-2 bg-gray-50 dark:bg-gray-800 rounded px-2 py-1">
                <div className="flex items-center gap-2">
                  <CoinIcon src={coin.image} alt={coin.name} />
                  <Link href={`/coin/${coin.id}`} className="font-semibold hover:underline text-xs sm:text-sm">
                    {coin.name}
                  </Link>
                ...
                <button
                  className={`px-2 py-1 rounded text-xs ${isWatched(coin.id) ? 'bg-yellow-300' : 'bg-gray-200 dark:bg-gray-700'}`}
      ...
      <style>{`
        @media (max-width: 640px) {
          .fixed.top-2.left-1\/2.-translate-x-1\/2.z-50.w-full.max-w-md.px-2 { max-width: 98vw; left: 50%; transform: translateX(-50%); }
        }
      `}</style>
    </div>
  );
